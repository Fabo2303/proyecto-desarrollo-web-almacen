...
import { chooseFetch, saveProduct } from "src/api/Product";
...
import { saveInput } from "src/api/Input";
import { saveInputDetail } from "src/api/InputDetail";
...
const TableEntrada = () => {
    ...
    const fetching = async () => {
        ...
        if (jwt) {
            findByUsername(jwt, userPayload.name).then((user) => {
                saveInput(jwt, {id: 0, date : new Date(), description: 'Entrada de productos', user: user}).then((input) => {
                    list.map((producto) => {
                        saveProduct(jwt, producto).then((product) => {
                            saveInputDetail(jwt, {id: 0, input: input, product: product, quantity: producto.stock}).then((inputDetail) => {
    ...
    return (
        <div className="relative w-full h-full">
            ...
            <ModalProduct isOpen={isOpenModal && modalType === 'Product'} onClose={closeModal} data={product || {}} columns={['sku', 'name', 'category', 'brand', 'expiration', 'stock']} title="Producto" nuevo={nuevo} guardarEnLista={guardarEnLista} origin={true}></ModalProduct>
            <ModalDocument isOpen={isOpenModal && modalType === 'Document'} onClose={closeModal} data1={list} headers={["sku", "name", "category", "brand", "expiration", "stock"]} title1="Entradas"/>
            <ModalListProduct isOpen={isOpenModal && modalType === 'List'} onClose={closeModal} title="Lista de entradas" data={list} onDataChange={onDataChange} handleDocument={handleDocument}/>
            <TableCustom data={data} columns={columns} handleSearch={handleSearch} rowClassName={rowClassName} btnfunction={handleButton} >
                {
                    isInput && <button onClick={handleList} className="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 focus:outline-none focus:bg-blue-600">Lista de entrada</button>
                }
            </TableCustom>
        </div>
    );
